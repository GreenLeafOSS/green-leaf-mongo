...
object EntityWithIdAsFieldDaoSpec:

  // **************************************************
  // MODEL
  // ID - is a single and simple field { "id": 1, "name": ... }
  // **************************************************
  final case class Building(id: Long, name: String, height: Int, floors: Int, year: Int, address: String)

  // **************************************************
  // DAO BSON PROTOCOL
  // **************************************************

  trait BuildingModelBsonProtocol extends GreenLeafMongoDaoProtocol[Long, Building]:
  ...
  abstract class BuildingDao extends TestGreenLeafMongoDao[Long, Building]:

    this: BuildingModelBsonProtocol with GreenLeafMongoJsonBasicFormats with GreenLeafJsonBsonOps =>

    def findByName(name: String): Future[Seq[Building]] =
      find("name" $regex (pattern = name, options = "i"))
    ...
    def findByAddressAndYear(address: String, year: Int): Future[Seq[Building]] =
      find($and("address" $regex (pattern = address, options = "i"), "year" $gte year))

abstract class EntityWithIdAsFieldDaoSpec extends TestMongoServer:

  import io.github.greenleafoss.mongo.core.dao.EntityWithIdAsFieldDaoSpec.*

  protected def newBuildingDao: BuildingDao

  // https://en.wikipedia.org/wiki/List_of_tallest_buildings_in_New_York_City#Tallest_buildings
  protected val BuildingsInNyc: Map[Long, Building] = Map(
  ...
  "BuildingDao (id as single field)" should:
    ...
    "insert one record" in:
      val dao = newBuildingDao
      for insertRes <- dao.insert(BuildingsInNyc(1))
      yield insertRes.wasAcknowledged shouldBe true

    "insert multiple records" in:
      val dao = newBuildingDao
      for insertRes <- dao.insertMany(Seq(BuildingsInNyc(2), BuildingsInNyc(3), BuildingsInNyc(4)))
      yield insertRes.wasAcknowledged shouldBe true
    ...
    "find by ids" in:
      val dao = newBuildingDao
      for
        insertRes <- dao.insertMany(Seq(BuildingsInNyc(6), BuildingsInNyc(7), BuildingsInNyc(8), BuildingsInNyc(9)))
    ...
    "find all" in:
      val dao = newBuildingDao
      for
        insertRes  <- dao.insertMany(BuildingsInNyc.values.toSeq)
    ...
    "findByName" in:
      val dao = newBuildingDao
      for
        insertRes     <- dao.insertMany(Seq(BuildingsInNyc(9), BuildingsInNyc(10)))
